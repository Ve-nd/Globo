    ...
    <div className="max-w-6xl mx-auto h-[calc(100vh-120px)] flex flex-col">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Manage and track live orders.</p>
        </div>
      </div>
      ...
      <div className="flex gap-2 p-1 bg-gray-100 rounded-xl w-full md:w-auto mb-6">
        {[
          { id: 'new', label: 'New', count: orders.filter(o => o.status === 'new').length, color: 'bg-red-500' },
          { id: 'preparing', label: 'Preparing', count: orders.filter(o => o.status === 'preparing').length, color: 'bg-yellow-500' },
          { id: 'ready', label: 'Ready', count: orders.filter(o => o.status === 'ready').length, color: 'bg-green-500' }
        ].map(tab => (
          <button
            key={tab.id}
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar">
        {filteredOrders.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-gray-400">
            ...
            <p className="text-sm">You're all caught up here.</p>
          </div>
        ) : (
